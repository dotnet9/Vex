...
| 导出 HTML |  | 导出独立 HTML 文件 | 文件可由浏览器打开，内容和基础样式正确，本地图片应按当前文档路径内联或可访问 |
| 导出 PDF |  | 导出图像型分页 PDF | 图片等资源需要嵌入 PDF 文件，通过邮件、QQ、微信等接收后也能正常预览格式、图片；后续完善文本可选择和更精细分页 |
| 导出 Word(doc\docx) |  | 后续实现 | 实现前状态栏明确提示待实现，图片等资源需要嵌入Word文件，通过邮件、QQ、微信等接收后也能正常预览格式、图片，Word格式规范，正常标书要求格式 |
| 导出 PNG |  | 导出当前 Markdown 为长图 | 图片等资源随渲染结果写入 PNG，长文档按长图策略输出 |
...
图片等资源需要嵌入 PDF 文件，通过邮件、QQ、微信等接收后也能正常预览格式和图片。
...
41. 导出菜单支持 HTML、PNG 和首版 PDF；PDF 当前为图像型分页文件，后续继续完善文本可选择、分页断点和精细排版。
42. 新手引导覆盖关键菜单、文档列表、大纲、编辑区、预览区和状态栏，且可从帮助菜单再次打开。
43. 深色模式已进一步覆盖查找栏、统计/关于/属性浮层、删除确认和未保存确认浮层。
44. 帮助菜单的内置文档会优先打开当前语言版本，英文快速开始和英文鸣谢文档已随程序输出。
45. Markdown 预览按当前文档路径解析相对图片，SVG 正常渲染，GIF 可播放；HTML/打印/复制会内联本地图片，PNG/PDF 导出会对本地 SVG 图片做栅格化。
46. 新手引导 Guide 的边缘对齐位置已通过本地 `CodeWF.AvaloniaControls` 修正，文件菜单等靠边步骤不再越界。
...
86. HTML/打印/复制与 PNG/PDF 导出解析本地图片时会同时尝试原始路径和 URL 解码路径，带空格等需编码的相对图片文件名不再容易导出缺图。
...
1. PDF 导出成熟化：文本可选择，继续补齐更复杂块级元素的排版映射；任务列表状态和表格单元格 inline 样式已纳入图像型 PNG/PDF 导出。
2. 原生打印或继续完善系统打印流程；当前 HTML 打印预览已具备主题化工具条、纸张/边距/页眉页脚控制、更稳定的打印 CSS，以及与 PDF/大纲一致的标题解析。
3. 深色模式细节完善：继续验证排版主题内容区域对比度；普通文本和右键菜单弹出边界已接入 Vex 动态主题资源。
4. I18n 文案完整迁移：继续补齐剩余边界提示；繁体中文/日文更新日志摘要、帮助菜单空 topic 英文兜底、打印/PDF 默认文件名兜底已更新。
5. 大文件性能优化：继续补充 UI 级真实超大文件压测；服务级压测脚本已覆盖 120,000 行 Markdown，大纲/PDF/打印标题扫描已有共享 span 逐行扫描，文件摘要边界已有有界流式扫描保护。
6. 安装包成熟化：MSIX 真实签名/安装验证和 MSI 方案按需补齐；压缩包发布产物、MSIX 布局/打包脚本与 PrepareOnly 冲突边界 smoke 已具备。
7. CodeWF.Markdown 可编辑预览控件与 Vex 可视化编辑模式。
