...
2. Markdown 排版主题使用参考`https://github.com/dotnet9/CodeWF.Markdown`，本地对应仓库目录`D:\github\CodeWF.Markdown`，完整支持10几种排版主题。
...
| 导出 PDF |  | 导出图像型分页 PDF | 支持本地相对图、`data:image`、HTTP(S)、SVG/GIF/WebP 等图片源，图片等资源嵌入 PDF 文件；导出成功后打开保存目录并定位文件；后续完善文本可选择和更精细分页 |
...
2. 剪贴板优先写入 `text/html` 或 Windows `HTML Format`；同时可提供 plain text 兜底，兜底内容也应是完整 HTML 片段，不应回退为 Markdown。
...
```html
<section id="vex" data-tool="markdown编辑器" data-website="https://codewf.com" style="font-size: 16px; color: black; padding: 25px 30px; line-height: 1.6; word-break: break-word; text-align: justify;">
  <h2 style="margin-top: 30px; font-weight: bold; font-size: 22px; border-bottom: 2px solid rgb(89,89,89); margin-bottom: 30px; color: rgb(89,89,89);">
    <span class="content" style="font-size: 22px; display: inline-block; border-bottom: 2px solid rgb(89,89,89);">这是标题</span>
  </h2>
  <p style="font-size: 16px; padding-top: 8px; padding-bottom: 8px; margin: 0; line-height: 26px; color: rgb(89,89,89);">这是内容</p>
</section>
```

8. 验收时必须把剪贴板内容粘贴到微信公众号正文编辑器，确认不是显示原始 HTML 文本，而是按富文本结构正常渲染。
...
PDF 导出不得假装文本可选择；当前图像型实现应在后续继续优化分页、页眉页脚和排版主题映射。
...
5. 主题切换后，Markdown 预览、HTML/打印导出、PNG/PDF 图像型导出和“复制到公众号”应使用同一套排版主题映射。
...
21. “复制到公众号”剪贴板内容能在微信公众号编辑器中以富文本方式正常渲染。
...
41. 导出菜单支持 HTML、PNG、Word 和首版 PDF；PDF 当前为图像型分页文件，后续继续完善文本可选择、分页断点和精细排版。
42. 新手引导覆盖关键菜单、文档列表、大纲、编辑区、预览区和状态栏，且可从帮助菜单再次打开。
43. 深色模式已进一步覆盖查找栏、统计/关于/属性/删除对话框、未保存确认和错误浮层。
44. 帮助菜单的内置文档会优先打开当前语言版本，英文快速开始和英文鸣谢文档已随程序输出。
...
66. 帮助文档对未知非中文语言优先回退英文文档，再回退简体中文；中文语言仍优先回退简体中文。
67. Markdown 统计的行数、段落、标题和横线统计合并为单次字符扫描，减少大文件逐行正则匹配开销。
68. HTML/打印、PNG 与图像型 PDF 导出会读取当前 Markdown 排版主题和紧凑布局，并通过共享导出样式映射应用颜色与字号。
...
76. 图像型 PDF 页面背景、页眉页脚元数据颜色和分页空白带检测会读取当前 `MarkdownExportStyle`；暗色排版主题不再用白底空白行作为断点判断。
...
85. PNG/PDF 图像型导出会识别任务列表状态，将 `- [ ]` 与 `- [x]` 渲染为 `[ ]`/`[x]` marker，减少复杂块级元素映射缺口。
...
89. PNG/PDF 图像型导出的表格单元格会渲染段落 inline，保留粗体、斜体、删除线、行内代码和链接样式，不再全部压平成纯文本。
...
95. 复制到公众号、知乎、稀土掘金会写入 HTML 剪贴板内容，元数据统一为 `markdown编辑器` 和 `https://codewf.com`。
...
3. 修复后 Vex 预览、HTML/打印、PNG/PDF 图像型导出、Word 导出和复制路径中的 SVG 处理策略应保持一致；无法共享实现时必须记录差异。
...
3. Vex 切换排版主题后，预览、HTML/打印、PNG/PDF 导出和复制到公众号使用同一主题结果。
4. 如果某个参考 CSS 中存在 Vex 当前不支持的样式能力，应记录降级策略，不能静默换成其他主题。

### 24.3 复制到公众号富 HTML

问题：当前“复制到公众号”未实现微信公众号可正常渲染的富文本效果，不能只复制 Markdown 原文。

实现要求：

1. Vex 菜单动作负责把当前 Markdown 转为公众号可粘贴的 HTML 片段，并写入剪贴板富 HTML 格式。
2. HTML 生成应优先复用 CodeWF.Markdown 的排版主题输出能力，避免 Vex 另写一套不一致的样式。
3. 根节点使用 `section#vex`，携带 `data-tool`、`data-website` 和 inline style；标题、段落、列表、表格、代码块、图片等节点也应内联关键样式。
4. 粘贴到微信公众号正文编辑器后应直接显示富文本排版，而不是显示 Markdown 或原始 HTML 文本。
...
3. 复制到公众号富 HTML：剪贴板写入微信公众号可正常渲染的 HTML 片段，而不是 Markdown 原文。
4. CodeWF.AvaloniaControls Guide 箭头增强：三角指示明显，目标控件粗边框清晰，Vex 使用本地 NuGet 包验证。
5. Vex 界面视觉质量专项：优先优化状态栏，并覆盖标题栏、侧边栏、查找栏、浮层和右键菜单。
6. PDF 导出成熟化：文本可选择，继续补齐更复杂块级元素的排版映射；任务列表状态、表格单元格 inline 样式和图片嵌入已纳入图像型 PNG/PDF 导出，Word 图片嵌入也已共用 `CodeWF.Markdown` 图片处理链路。
