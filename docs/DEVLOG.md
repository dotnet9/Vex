...
- 导出公共能力继续下沉：`CodeWF.Markdown` 新增 `MarkdownDocumentExporter`、`ExportKind`、`MarkdownExportDocument` 和公共 `MarkdownExportStyle`，封装 Markdown 字符串/文件到 PNG、可选择文本 PDF、Word `.docx` 的一站式导出；Vex 更新到本地 NuGet 包 `CodeWF.Markdown`/`CodeWF.Markdown.Themes` 12.0.3.14 后，PDF/PNG/Word 菜单统一调用 `ExportKind` 入口，只负责选文件和传当前主题，不再保留本地 `MarkdownPngRenderer`、`MarkdownPdfRenderer`、`MarkdownDocxExporter`。
...
- 验证本地打包生成 `CodeWF.Markdown.12.0.3.14.nupkg` 和 `CodeWF.Markdown.Themes.12.0.3.14.nupkg`，`dotnet test CodeWF.Markdown.slnx --no-restore -m:1` 48 项通过，Vex 使用本地包源构建 `Vex.slnx` 通过。
- 自媒体复制链路修复：`CodeWF.Markdown` 新增 `MarkdownHtmlClipboard`、`MarkdownHtmlClipboardExtensions` 与自媒体 profile，并本地打包为 12.0.3.14；Vex 通过本地包源引用 `CodeWF.Markdown`/`CodeWF.Markdown.Themes` 12.0.3.14，不使用跨仓库项目引用。Windows `HTML Format` 改为 UTF-8 CF_HTML 字节载荷，公众号后台不再把 HTML 片段当普通文本显示。
...
- 验证 `CodeWF.Markdown.12.0.3.14.nupkg` 等本地包；`dotnet test CodeWF.Markdown.slnx --no-restore -m:1` 48 项通过，`dotnet build Vex.slnx --no-restore -m:1` 和 `git diff --check` 通过。
...
- 社交复制重写为目标平台 HTML 片段，公众号、知乎、稀土掘金剪贴板内容使用多语言 `data-tool` 和 `data-website="https://codewf.com"`，掘金追加本地化排版后缀。
...
- 验证 `dotnet build Vex.slnx -v:minimal`，并用源码结构 smoke 确认打印预览 CSS 由 `BuildPrintPreviewStyles` 动态生成，包含主题背景、正文色、边框色、链接色和分页保护规则。
...
- 修复新手引导 Guide 方位：`CodeWF.AvaloniaControls` 已切换为 NuGet 官方源 12.0.3.6，按 Avalonia anchor/gravity 语义修正 `TopLeft`、`RightTop`、`LeftTop` 等边缘对齐模式。
...
- Vex 从 NuGet 官方源还原 CodeWF 12.0.3.6 包，并移除仓库级 `.local-nuget` 本地包文件和临时 `NuGet.config`；验证 `dotnet build Vex.slnx -v:minimal`、NuGet 漏洞扫描，并用临时反射 smoke 确认 HTML 输出会内联相对 GIF/SVG/PNG 且保留远程图片 URL。截图确认 Markdown SVG/GIF 预览正常且 GIF 像素变化可见。截图路径：`%TEMP%\VexScreenshots\media-svg-gif-preview.png`、`%TEMP%\VexScreenshots\media-svg-gif-preview-2.png`。
...
- `CodeWF.AvaloniaControls`/`CodeWF.AvaloniaControls.Themes` 已切换为 NuGet 官方源最新 12.0.3.6，还原不再依赖仓库级 `.local-nuget` 源。
- 移除临时 `NuGet.config` 本地包源配置和 `.local-nuget` 本地包文件，避免继续使用本地打包兜底。
...
- Moved web HTML paste conversion into `CodeWF.Markdown`: the package now includes `MarkdownHtmlConverter` and `MarkdownHtmlClipboard.Html2Markdown(string htmlContent)`. Vex reads clipboard HTML first when pasting into the middle editor, inserts converted Markdown, and falls back to the native AvaloniaEdit paste path when HTML is unavailable or conversion fails.
- Vex now references locally packed `CodeWF.Markdown`/`CodeWF.Markdown.Themes` 12.0.3.14. `Ctrl+V`, `Command+V`, and menu paste all route through the editor action service. Verified 48 `CodeWF.Markdown.Tests` pass, repacked the local NuGet packages, forced Vex restore, and built `Vex.slnx` successfully.
- Continued moving export capabilities into `CodeWF.Markdown`: the package now provides `MarkdownDocumentExporter`, `ExportKind`, `MarkdownExportDocument`, and public `MarkdownExportStyle` APIs for Markdown string/file export to PNG, selectable-text PDF, and Word `.docx`. After updating to local NuGet packages `CodeWF.Markdown`/`CodeWF.Markdown.Themes` 12.0.3.14, Vex export menu actions call the unified `ExportKind` entry point, choose the save path, and pass the active theme; local `MarkdownPngRenderer`, `MarkdownPdfRenderer`, and `MarkdownDocxExporter` were removed from Vex.
...
- Verified local packing produced `CodeWF.Markdown.12.0.3.14.nupkg` and `CodeWF.Markdown.Themes.12.0.3.14.nupkg`, `dotnet test CodeWF.Markdown.slnx --no-restore -m:1` passed 48 tests, and Vex built against the local package source.
- Fixed the social-copy pipeline: `CodeWF.Markdown` now includes `MarkdownHtmlClipboard`, `MarkdownHtmlClipboardExtensions`, and social copy profiles, and was packed locally as 12.0.3.14; Vex consumes `CodeWF.Markdown`/`CodeWF.Markdown.Themes` 12.0.3.14 from the existing local package source, without a cross-repository project reference. Windows `HTML Format` is now UTF-8 CF_HTML bytes, so WeChat's editor no longer receives the HTML fragment as plain text.
...
- Verified `CodeWF.Markdown.12.0.3.14.nupkg` and related local packages; `dotnet test CodeWF.Markdown.slnx --no-restore -m:1` passed 48 tests, `dotnet build Vex.slnx --no-restore -m:1`, and `git diff --check` passed.
...
- Built `Vex.slnx` and used a source-structure smoke to verify print-preview CSS is generated by `BuildPrintPreviewStyles` with themed background, body color, border color, link color, and page-break protection rules.
...
- Fixed Guide placement by restoring `CodeWF.AvaloniaControls` 12.0.3.6 from the official NuGet source and mapping edge-aligned modes such as `TopLeft`, `RightTop`, and `LeftTop` to the matching Avalonia anchor/gravity behavior.
...
- Vex restores the CodeWF 12.0.3.6 packages from the official NuGet source and no longer ships local `.local-nuget` package files or a temporary `NuGet.config`; verified `dotnet build Vex.slnx -v:minimal`, completed NuGet vulnerability scanning, and used a temporary reflection smoke to confirm HTML output inlines relative GIF/SVG/PNG while preserving remote image URLs. Screenshot-checked Markdown SVG/GIF preview with visible GIF pixel changes. Screenshot paths: `%TEMP%\VexScreenshots\media-svg-gif-preview.png`, `%TEMP%\VexScreenshots\media-svg-gif-preview-2.png`.
...
- Switched `CodeWF.AvaloniaControls` and `CodeWF.AvaloniaControls.Themes` to the latest nuget.org 12.0.3.6 packages, so restore no longer depends on the repository `.local-nuget` source.
- Removed the temporary local-source `NuGet.config` and local `.local-nuget` package files to avoid relying on locally packed fallback packages.
